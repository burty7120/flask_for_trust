...
def get_transactions():
    if request.method == 'OPTIONS':
        logging.debug("Обробка OPTIONS-запиту для /get_transactions")
        return '', 204
    try:
        user_id = request.args.get('user_id')
        user = db.session.get(User, user_id)
        if user:
            transactions = Log.query.filter_by(user_id=user_id).all()
            transaction_data = [{'amount': tx.amount, 'asset': tx.asset, 'action': tx.action, 'timestamp': tx.timestamp.isoformat()} for tx in transactions if tx.amount is not None]
            log_action(user.id, 'Viewed transactions')
            logging.debug(f"Транзакції отримано: user_id={user_id}")
            return jsonify({'success': True, 'transactions': transaction_data})
        logging.warning(f"Користувача не знайдено: user_id={user_id}")
        return jsonify({'success': False, 'message': 'Користувача не знайдено'})
    except Exception as e:
        logging.error(f"Помилка в /get_transactions: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500
